...
public class ImagePageProcesser extends abstractPageProcesser {

    private static String domain = "http://www.bapilang.link";
    private static String fileDirectoryPath = "E:\\crawler\\data\\webmagic\\image\\";
    private static String startUrl = "http://www.bapilang.link/tchy/13/1.html";//"http://www.bapilang.link/rzqhkb/443411.html";//"http://www.bapilang.link/rzqhkb/432524.html";//"http://www.bapilang.link/rzqhkb/431074.html";//
    ...
    public void process(Page page) {
		...
		String naviRegex = "(/tchy/13/\\w{1,6}\\.html)";
		String detailRegex = "(/rzqhkb/\\w{1,6}\\.html)";
		
    	if(page.getUrl().regex(naviRegex).match()){//遍历导航及明细链接
    		log("["+currentUrl + "] is navi url match [" + naviRegex + "]");
        	List<String> naviUrls = page.getHtml().links().regex("(/tchy/13/\\w{1,6}\\.html)").all();//一级导航
        	log("discover navi urls~~~"+ naviUrls.size() + "~~~" + naviUrls.toString());
	       	List<String> detailUrls = page.getHtml().links().regex("(/rzqhkb/\\w{1,6}\\.html)").all();//二级明细
	    	log("discover detail urls~~~"+ detailUrls.size() + "~~~" + detailUrls.toString());
    		...
    		log("["+currentUrl + "] is detail url match [" + detailRegex + "]");
    		//记录导航及明细页面标题
        	...
        	log("discover image urls~~~"+ imageUrlList.size() + "~~~" + imageUrlList.toString());
